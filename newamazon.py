...
RESPONSE=
	...
	cmdarray=cmdintermediate.choices[0].message.content to array on the basis of newlines
	
	#loop to feed context
	for currprompt in allprompts:
		garbageresponse=client.chat.completions.create(model="gpt-3.5-turbo",messages=[{"role": "user", "content": currprompt}])# temperature=0,frequency_penalty=-2.0, presence_penalty=-2.0,  don't alter top_p
	...
	try:
		for currcmd in cmdarray:#individual steps fed into gpt with each iteration
			browser_content = "\n".join(_crawler.crawl())
			prev_cmd = gpt_cmd
			gpt_cmd = get_gpt_command(currcmd, _crawler.page.url, prev_cmd, browser_content)
